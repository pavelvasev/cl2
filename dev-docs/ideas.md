...
если тип не определен то отложить компиляцию этого узла в очередь.
и повторить в конце. тип уже определиться скорее всего.
делать так пока очередь например не перестанет уменьшаться (во 2й и далее проход)

************
если бы добавить список "известных пакетов" (т.е. их адреса) то тогда можно говорить clon dom init.
и здесь init это команда прямо пакета dom.

а можно и другие команды, разные.

***********************

- лучше бы сделал хуку "процесс создали" или типа того.
  кстати вот это интересно кстати так-то.
  !!!!!!!!!!

ну т.е. по сути добавить в язык возможности разговора с компилятором,
как парсер дал возможности разговора с парсером...  

хотя у нас и есть init но это на этапе как бы рантайма.
а хотелось бы co_init - на этапе кодогенерации.
и например с возможностями какими-то.

***********************
ну и вот интересно например 
  left_args 1 2 3 { print .... print ...} и типа оно припишет всем вложенным аргументы.. ну если это часто надо вообще говоря..

  но в целом тут не приписывание аргументов наверное а замена функции. хотя мб и приписывание.
вот это интересно.

а еще больше вот это интересно:
...
пусть даже и в статике, но это вроде как прикольно. чувак об этом рассказывал о контексте. вот эт идея.

идея повторюсь в том чтобы неявно но мочь заменить вещи. хотя может стоит и явно. и тогда мб даже проще будет.

*************
clon init <имяпакета>
  и вызывается команда инит у имяпакета. а пакет резолвится по таблице известных пакетов.
  ну скачивается видимо куда-то.  

*************  **************

надо все-таки проникнуться идеей кодогенерации. как-то вникнуть в нее понять.
пример.
...
и вот я хочу заделать аналогичное - row. сейчас я занимаюсь копи-пастой.
а надо бы что-то вроде:

function generate_gr( type ) {
  return clon`obj "column" { ... ${type} ... }`
}

obj "column" [generate_gr "column"]
obj "row" [generate_gr "row"]

1 вот может путем dom template пойти.
2 или путем erb..
3 или путем LF - они внедрили подстановку параметров и еще ограниченную кодогенерацию на уровне особых форм и синтаксиса, типа cell [n]

<% def mk() return "" end %>
<%= mk("column") %>

это кстати прикольно, свобода вообще где угодно чего угодно. надо подумать об этом.
может даже типа ejs приплести. нам то какая разница. бесплатная кодогенерация!
но только на уровне текстов. т.е. не напишешь вещи типа x := draw "points"
...
%mkitem "column" "column"

**************************
идея что надо идти последовательно. вот у нас 3 уровня.
надо хорошенько проработать первый для начала. т.к. 2й ну ок а 3й может и не понадобится.

в этом смысле задачи 1го уровня имеют выше приоритет.

**************************
реакт неудобный сейчас.
пусть мне надо реагировать когда два свойства заданы
сейчас это вот так
  react (list @map @interactions) {: args |
    let map = args[0]
    let arr = args[1]
    arr.forEach( x => map.addInteraction(x);)
  :}
и это неправильно.

по лф-ски будет так кстати:

  react @map @interactions {: map arr |
    arr.forEach( x => map.addInteraction(x);)
  :}
но тут подразумевается any. мне надо бы с этим разобраться.
варианты:
- react @map @arr all=true
- договориться как-то на уровне синтаксиса.  

*************
openlayers очень удобно что есть 300 примеров и по ним поиск.
но вот апи неудобно аписано. а примеры удобно.

*************
генератор определений объектов. т.е. задаем генератор в пр-ве sl
а потом пишем sl.button и оно генерит что надо.

-----
мысль такая что - если оставить только уровень 1, то может людям будет легче это все воспринимать.. ну и сама система как бы существенно проще становится.
у нее даже ссылки можно убрать...
=======
вызов функции - это вообще говоря то же самое что посылка сообщения процессу но с динамической маршрутизацией - мы же результат "возвращаем".

а так у объектов она обычно же статическая - вот входы, вот выходы.
а функции позволяют такую динамику делать. так-то это оч круто.
надо это понять.

... в этом смысле вызов функции можно рассматривать как альтернативу посылке сообщению в канал. оно в каком-то смысле им и является. ну и можно рассматривать что мы "вызываем" ее а она внутри там посылает..
*****************
в LF bind это -> а у нас это @ на самом то деле. т.е. любая ссылка на ячейку это и есть bind

*****************
я все больше прихожу к идее что надо больше внимания уделить именно уровню 1.
чтобы он был красив, лаконичен. и ясен.
и тогда пусть "внутри" (в коде реакций) и "во-вне" (в либах) творится все что угодно. если мы обрамляем это в ясную и четкую структуру - то и все хорошо.

*****************
подключить js-linter - он проверяет коды что там все правильно.

********
мб в режиме c++ заняться интерпретацией.. т.е. реакции и апплай работают в кодах.
а вот под-функции {}...

============
оператор подстановки.. он кстати просто особая форма
subst "a b c @alfa @beta"
но ему потом мб туда поинтереснее вещи добавить..
- скобки поинтереснее, 
- операторы?.. кстати вообще любые пайповые например.. или просто выражения..
- ну т.е. суть что мы чуть меняем форму записи, т.к. итоговый объект строка.


================
надо в слона научиться обычные добавлять вещи на js написанные.
ну вот заданные классом или функцией неважно. можно даже типы не проверять или затребовать сигнатуры для проверки. ну тогда мы сдвигаемся и не все уже на слоне а слон требует апи стыковки - ну вот что там каналы и др. точки синхронизации. хотя как мы знаем самое главное каналы. но вообще вот и вызов метода это важно..

ну т.е. вот класс.. и он сделал доп члены - каналы.
но может и не каналы а какие-то вещи у которых есть subscribe и submit.

ну или еще как или что-то такое. но в итоге чтобы и правда, можно было на любом языке js-среды написать. не все же слон.

но впрочем это частично почти есть - мы же вызвыаем функцию при создании объекта..

===================
func register_bot( bot )  таки перейти к стандарту записи функций общепринятому

*******************
сделать ТПУ на реакциях. для этого ввести новый примитив.
типа action в ЛФ только ТПУ.
ну и реакции при исполненнии.. например через этот тпу имеют возможность управлять цепочкой.

*******************
F-STREAM

поток это канал но с флагом "завершен". ну или ячейка, надо подумать.
реализация - канал/ячека + ячейка "finish".

ну и при подписке передача 2х вещей - значения + признака финиш или нет.
чтобы последующие каналы тоже могли сказать мол финишь.

**********
попытка сделать версию для ноды-10.
npx esbuild cl-tool.js --bundle --platform=node --target=node10 >tool10.mjs

***************
мб свой пакетный менеджер это и лишнее. равно как и тестер.
рацио - от npm никуда не уйдешь, все-равно внешние пакеты.
но и контр-рацио - сейчас npm завтра yarn и тп и тп.
ну и в разных проектах разное.
но с другой стороны это на усмотрение пользователя +
можно оставить наш стандартный мета-файл.. в общем можно подумать об этом.

**********************

попробовать ввести последовательный режим для реализации функций.
т.е. сейчас все принудительно процессное но это проблема. даже для простых кодов я всегда ухожу в js.

button "bla" {
  submit @channel (@some + @value)
  print "data sent"
}

update или не последовательный но хотя бы с гарантией что вычисления выполнятся 1 раз.
именно это требуется в первую очередь.
последовательность можно промоделировать оператором =======.

в общем нужны эксперименты. что надо, чего достаточно.

------
внедренный css бы.. чтобы действовал ток на элемент к которому прицеплен. но через цсс-селекторы чтобы.
это удобно когда поля style недостаточно.

************
надо запускалку шаблонов проектов в духе npm сделать..
    но с другой стороны это надо микро-репозиторий проектов уже иметь..
или прямо в нмп вписаться: https://petermekhaeil.com/how-to-build-an-npx-starter-template/
но минус что нпм сегодня есть, завтра нет. или другой менеджер захотим.

и это связано с тем что хорошо бы мб не clon add а npm install..

-----------
repeater input=@shared_view[type="combobox"] а хорошая штука css была. и питон молодец.
...
-----
где-то я писал но не вижу - синтаксические селекторы данных..
т.е. @somedata.alfa.beta.gamma - проходка по пути.. 
и плюс массовый селектор в духе @some.alfa_array.*.beta.gamma

-----------
надо уже сделать чтобы в 1 файле и слон-сервер и слон-клиент. это хорошо будет. веб-приложение в 1 файле (проекте) это может оказаться реально удобно.
а если еще и запускалку в облаке к нему приделать - прям песня.
clon-submit . и вуаля - все в облаке твое.
но может проще через гит пойти и пусть в облако само все тянется.
можно даже так - настраиваешь себе облако 1 раз свое - и оно туда тянется.

ну у серверных приложений кстати проблема - накладные расходы. и они решаются массовостью (1 процесс на всех клиентов условно) и хранением состояния в бд..

-----
пишу си программу.
кстати мне было бы удобно на уровне макросов:
- напечтать что-то на экран во время компиляции
например что ифдев не сработал.
- сгенерировать блин программу. строчкой. и сказать - добавляй.
вот это было бы вообще удобно.
т.о. эдакий <% .. %> на этапе компиляции. хмхмхмх.