...
итого F-PACKAGE-JSON
- в дополнение к clon.mjs читаем пакеты из package.json.

из этого следует что пакеты мы не можем уже управлять именами как в clon.mjs.
и сообразно ссылки будут вида import dom="dom.cl"
и как следствие вопрос - может тогда пакеты называть не *.cl а что-то вида *.clp?
клопы какие-то))) ну или *.cp
)))
