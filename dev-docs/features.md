...
todo: delayed это некорректное название
...
update - это убрано. теперь реакции не возвращают значений.
вычисление значений переместилось в apply.
а вот там да.. надо что-то думать мб аналогичное будет..

и к реакциям вопрос открытый - стоит ли им дожидаться завершения процесса или нет.
возможно, что выстраивание по очереди и ожидание завершение процесса - это какой-то
другой примитив.
...
**************
F-PIPE-INPUT
а пусть пайпа кладет не в 1й аргумент а в input.
это удобно. и можно для apply применять. и для других.

получилось. но это конечно.. усложнение семантики пайпы. ну посмотрим. зато apply теперь можно будет сделать. ну если input ему прописать.

F-PIPE-APPLY
но для апплай не получилось сделать - введение в апплай поля input дает его серьезное усложениние.
поэтому для пайпы введено правило-исключение что в апплай оно кладет в 1й аргумент а остальные сдвигает. такое.

*************
F-THENABLE
в объекты синхронизации добавлен метод .then равный по логике .once
и это делает их then-able, т.е. промисы яваскрипт могут поедать их.
Пример: return Promise.resolve( some_cell )
вернет обещание котороеы разрезолвится значением ячейки (когда оно будет назначено)
...
***********************
F-MAY-BIND-TO-OBJECT можно делать бинды на объекты т.е. связывание объекта и ячейки. Это значит надо положить в ячейку объект.