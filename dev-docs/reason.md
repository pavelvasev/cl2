...
```
Выглядит красиво, хвала создателю unix pipes. Но кстати здесь даже богаче - есть явные связи между процессами (radius=@c.output).
...
3. А нужна ли в принципе была эта история, если можно использовать декораторы (в духе Питон) как генераторы процессов? Например порождать действия запускающиеся по готовности данных, с параметром - указанной функцией. Но с другой стороны мы описали процесс, а затем ведь его надо использовать и увязывать с другими процессами. Но можно же писать в духе примеров Лисп, передавать каналы в функции как параметры и там уже пусть с ними связываются в реализации функции, получившей управление.
4. При этом учитываем что классы в языках программирования - это на самом деле синтаксис и модель для генерации функции, которая порождает структуру данных согласно описанию данному в классе.
