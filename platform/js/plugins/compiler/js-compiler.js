...
export function default_obj2js( obj,state ) {
	...
	let {normal, renamed, pos_rest,named_rest,children_param, 
	    pos_splat, named_splat, const_params} = basis_record.check_params( assigned_names, obj.params, obj.locinfo )
	...
	let links_names = Object.keys(obj.links)
		...
		for (let k of links_names) {
			...
			if (const_params[ link.to]) { // F-CONST-PARAM
				if (!state.static_values[ link.from ])
				{
					console.error("param: setting by non-static value!")
					console.error( obj.locinfo )
					throw new Error( "param: setting by non-static value!" )
				}
			}
  ...
  // process "alfa" comm=true значит коммуникатор и не надо добавлять в статические имена
  if (!obj.name_is_autogenerated && !basis_record.obj_record.params.comm) {
	  ...
	  for (let k in basis_record.get_const_params()) 
	  {
	  	// console.log("setting nama",`${objid}.${k}`)
	  	state.static_values[ `${objid}.${k}` ] = true
	  }

	  //console.log("attached static value")
  }
  ...
  if (state.struc_parent_id && !obj.skip_attach) {
	...
	// , static_obj_id: (obj.name_is_autogenerated ? objid : null)
}
